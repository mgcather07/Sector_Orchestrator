...
| [0006](0006-android-roster-order-and-host-add.md) Android roster order + host add-team | tournament-registration | Android_Sector | android | feature | **approved** (2026-08-24) | Parity with **iOS PR #191**: number roster by registration order + sort filter; host adds a team for people without a smart device. **Unique id per host add** — the deterministic `{tid}_{uid}` id would overwrite. |
